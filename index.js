...
function findIndexByProperty(arr, name) {
  ...
  const result = arr.findIndex((element) => element.name === name);
  return result;
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flattenedAndSquaredArray = arr.flat().map(element => element * element);
  const filteredSquaredArray = flattenedAndSquaredArray.filter(element => element > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const NewArr = arr.slice(-n); // Отримуємо останні n елементів
  const sum = NewArr.reduceRight((accumulator, value) => accumulator + value, 0); // Обчислюємо суму останніх n елементів

  NewArr.forEach(element => console.log(element)); // Виводимо останні n елементів в консоль
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap(element => {
    const repetitions = new Array(n).fill(element);
    return repetitions;
  });
  return expandedArray;
  // Перевіряємо, чи вхідні параметри є масивом та числом відповідно, якщо ні повертаємо 
